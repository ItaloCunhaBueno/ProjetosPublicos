...
class GUIDraw(QWidget):
    ...
    def save_result(self):
        path, file = os.path.split(os.path.abspath(self.image_file))
        ext = file[-3:]
        file = file[0:-4]
        print("File saved to: {0}".format(os.path.join(path, '{0}_result.{1}'.format(file, ext))))
        cv2.imwrite(os.path.join(path, '{0}_result.{1}'.format(file, ext)), self.model.get_img_fullres()[:, :, ::-1])
