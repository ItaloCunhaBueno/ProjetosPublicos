...
while 1:
    ...
    if event == "con_lista_cat":
        cur = conn.cursor()
        SQL_CATS = cur.execute("select categoria from receitas")
        LISTA_CATS = []
        for ING in SQL_CATS:
            if ING[0]:
                INGS = [
                    LISTA_CATS.append(IN)
                    for IN in ING[0].split(";")
                    if IN not in LISTA_CATS
                ]
        window["con_cat_listbox"].Update(LISTA_CATS)
        window.Refresh()
    ...
    if event == "con_cat_listbox":
        if values["con_cat_listbox"]:
            VALUE = values["con_cat_listbox"][0]
            cur = conn.cursor()
            SQL_RECEITAS = cur.execute(
                f"select id, nome from receitas where categoria like '%{VALUE}%'"
            )
            LISTA_RECEITAS = []
            for REC in SQL_RECEITAS:
                if f"{REC[0]}. {REC[1]}" not in LISTA_RECEITAS:
